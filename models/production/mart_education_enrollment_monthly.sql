...
trim(academic_year) as academic_year  FROM {{source('staging', 'education_ngo_airtable_source_xlsx_raw_students_tbl5xi_70289669')}} cross join lateral generate_series(date_trunc('month', enrollment_date_text)::date, date '2026-07-01', interval '1 month') as gs(snapshot_month))
