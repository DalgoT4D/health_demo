...
date_trunc('month', case when trim(enrollment_date_text) ~ '^[0-9]{4}-[0-9]{1,2}-[0-9]{1,2}$' then make_date(split_part(trim(enrollment_date_text),'-',1)::integer,split_part(trim(enrollment_date_text),'-',2)::integer,split_part(trim(enrollment_date_text),'-',3)::integer) else make_date(2025 + ((coalesce(nullif(regexp_replace(student_id,'[^0-9]','','g'),''),'1')::integer % 2)), ((coalesce(nullif(regexp_replace(student_id,'[^0-9]','','g'),''),'1')::integer - 1) % 12) + 1, 1) end)::date as enrollment_month,
